...
"""CHANGELOG
- build ships if < other player with most ships
- evade if other player has > 50 ships
- est wait time set to 2
- fixed bug in call_home causing infinite WAIT
"""
...
class MyAgent:
    ...
    def move_to_target(self, ship, pt):
        ...
        actions['WAIT'] = actions['WAIT'] - 1 if maybe_yard is not None else actions[
            'WAIT']  # Penalty for waiting about on yards
        ...
        while chosen_action == 'UNDECIDED':
            ...
            for action in best_to_worst_actions:
                ...
                conditions['is_not_waiting_on_yard'] = (True if action != 'WAIT' else
                                                        ship.log.role != 'DEP' and ppos not in [sy.position for sy in
                                                                                                self.me.shipyards])
                conditions['is_not_occupied_by_enemy_yard'] = not (
    ...
    def determine_best_action(self, ship):
        if ship.log.role == 'HVST':
            ...
            if ship.halite > 0 or len(
                    self.get_adj_enemies(sy.position, radius=1)) > 0:  # otherwise move towards nearest enemy ship
    ...
    def assign_role(self, ship):
        ...
        any_e_has_40_ships = any(
            [len(p.ships) >= magic['evade_ship_count'] for p in self.board.players.values() if p.id != self.me.id])
    ...
    def start_of_turn_yard_spawning(self):
        ...
        if self.board.step > 20 and self.board.step < magic['end_game_step'] and self.me.halite > 2000 and len(
                self.me.ships) > 10 and len(self.me.shipyards) == 1:
            ship = self.get_best_ship_for_yard()
            if ship is not None:
                ship.next_action = ShipAction.CONVERT
                ship.log.set_action = ShipAction.CONVERT
    ...
    def setup_stats(self):
        ...
        LOG.enemy_targ_ids = [s.id for s in self.board.ships.values() if s.player_id != self.me.id] + [sy.id for sy in
                                                                                                       self.board.shipyards.values()
                                                                                                       if
                                                                                                       sy.player_id != self.me.id]
