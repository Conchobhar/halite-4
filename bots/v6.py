...
np.seterr(all='raise')
"""CHANGELOG
- global overharvest condition to avoid mining low halite zones
- uses full map to select harvpos, assumes shorter harvest time (3)
- ships release spot when depositing
- movement conditions role dependant
- tripswitch removed
"""
        ...
        while chosen_action == 'UNDECIDED':
            ...
            for action in best_to_worst_actions:
                ...
                conditions['is_not_waiting_on_yard'] = (
                        ship.log.role != 'DEP' and action == 'WAIT' and ppos not in [sy.position for sy in
                                                                                     self.me.shipyards])
